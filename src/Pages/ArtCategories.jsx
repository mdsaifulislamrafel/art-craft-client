            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {
                    category.slice(1, 7)?.map(item => <div data-aos="fade-up" data-aos-duration="3000"  key={item._id} className="w-full mx-auto space-y-6 rounded-2xl bg-[#e9d5ff] px-6 py-4 shadow-md  md:w-[500px]">
                        ...
                        <Link to={`/categories/${item.subcategory}`} ><button className="rounded-lg mt-4 bg-slate-800 px-6 py-2 text-[12px] font-semibold text-white hover:bg-slate-900 sm:text-sm md:text-base">See more</button></Link>
